...
+ (NSString *)zcnetworking_md5:(NSString *)string;
...
static ZCResponseType sg_responseType = kZCResponseTypeJSON;
static ZCRequestType  sg_requestType  = kZCRequestTypePlainText;
static ZCNetworkStatus sg_networkStatus = kZCNetworkStatusReachableViaWiFi;
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/ZCNetworkingCaches"];
}
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"ZCNetworking clear caches error: %@", error);
        } else {
            NSLog(@"ZCNetworking clear caches ok");
        }
    }
}
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(ZCURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[ZCURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (void)configRequestType:(ZCRequestType)requestType
             responseType:(ZCResponseType)responseType
...
+ (ZCURLSessionTask *)getWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                          success:(ZCResponseSuccess)success
                             fail:(ZCResponseFail)fail {
...
+ (ZCURLSessionTask *)getWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                           params:(NSDictionary *)params
                         progress:(ZCGetProgress)progress
                          success:(ZCResponseSuccess)success
                             fail:(ZCResponseFail)fail {
...
+ (ZCURLSessionTask *)postWithUrl:(NSString *)url
                      refreshCache:(BOOL)refreshCache
                            params:(NSDictionary *)params
                           success:(ZCResponseSuccess)success
                              fail:(ZCResponseFail)fail {
...
+ (ZCURLSessionTask *)postWithUrl:(NSString *)url
                      refreshCache:(BOOL)refreshCache
                            params:(NSDictionary *)params
                          progress:(ZCPostProgress)progress
                           success:(ZCResponseSuccess)success
                              fail:(ZCResponseFail)fail {
    return [self _requestWithUrl:url
                    refreshCache:refreshCache
...
+ (ZCURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(ZCDownloadProgress)progress
                               success:(ZCResponseSuccess)success
                                  fail:(ZCResponseFail)fail {
    ...
    ZCURLSessionTask *session = nil;
    
    if (httpMethod == 1) {
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == kZCNetworkStatusNotReachable ||  sg_networkStatus == kZCNetworkStatusUnknown ) {
                    id response = [ZCNetworking cahceResponseWithURL:absolute
                                                           parameters:params];
                    if (response) {
                        if (success) {
...
+ (ZCURLSessionTask *)uploadFileWithUrl:(NSString *)url
                           uploadingFile:(NSString *)uploadingFile
                                progress:(ZCUploadProgress)progress
                                 success:(ZCResponseSuccess)success
                                    fail:(ZCResponseFail)fail {
...
+ (ZCURLSessionTask *)uploadWithImage:(UIImage *)image
                                   url:(NSString *)url
    ...
    AFHTTPSessionManager *manager = [self manager];
    ZCURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (ZCURLSessionTask *)downloadWithUrl:(NSString *)url
                            saveToPath:(NSString *)saveToPath
                              progress:(ZCDownloadProgress)progressBlock
                               success:(ZCResponseSuccess)success
                               failure:(ZCResponseFail)failure {
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            DLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
    ...
    @synchronized (self) {
        ...
        if (sg_sharedManager == nil || sg_isBaseURLChanged) {
            ...
            switch (sg_requestType) {
                case kZCRequestTypeJSON: {
                    manager.requestSerializer = [AFJSONRequestSerializer serializer];
                    break;
                }
                case kZCRequestTypePlainText: {
            ...
            switch (sg_responseType) {
                case kZCResponseTypeJSON: {
                    manager.responseSerializer = [AFJSONResponseSerializer serializer];
                    break;
                }
                case kZCResponseTypeXML: {
                    manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
                    break;
                }
                case kZCResponseTypeData: {
...
+ (void)detectNetwork {
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            sg_networkStatus = kZCNetworkStatusNotReachable;
        } else if (status == AFNetworkReachabilityStatusUnknown){
            sg_networkStatus = kZCNetworkStatusUnknown;
        } else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            sg_networkStatus = kZCNetworkStatusReachableViaWWAN;
        } else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            sg_networkStatus = kZCNetworkStatusReachableViaWiFi;
        }
    }];
}
...
+ (void)successResponse:(id)responseData callback:(ZCResponseSuccess)success {
    if (success) {
        success([self tryToParseData:responseData]);
    }
}

+ (NSString *)zc_URLEncode:(NSString *)url {
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        NSString *key = [NSString zcnetworking_md5:absoluteURL];
...
+ (void)handleCallbackWithError:(NSError *)error fail:(ZCResponseFail)fail {
    if ([error code] == NSURLErrorCancelled) {
