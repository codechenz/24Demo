...
/// 相册展示内容的类型
typedef NS_ENUM(NSUInteger, ZCAlbumContentType) {
    ZCAlbumContentTypeAll,                                  // 展示所有资源（照片和视频）
    ZCAlbumContentTypeOnlyPhoto,                            // 只展示照片
    ZCAlbumContentTypeOnlyVideo,                            // 只展示视频
    ZCAlbumContentTypeOnlyAudio  NS_ENUM_AVAILABLE_IOS(8_0) // 只展示音频
};

/// 相册展示内容按日期排序的方式
typedef NS_ENUM(NSUInteger, ZCAlbumSortType) {
    ZCAlbumSortTypePositive,  // 日期最新的内容排在后面
    ZCAlbumSortTypeReverse  // 日期最新的内容排在前面
};


@interface ZCAssetsGroup : NSObject
 ...
 *                          可以以此作为判断枚举结束的标记
 */
- (void)enumerateAssetsUsingBlock:(void (^)(ZCAsset *result))enumerationBlock;
