...
@interface ZCAsset ()
@property (nonatomic, assign, readwrite) ZCAssetType assetType;
@end
        ...
        switch (phAsset.mediaType) {
            case PHAssetMediaTypeImage:
                if (phAsset.mediaSubtypes & PHAssetMediaSubtypePhotoLive) {
                    self.assetType = ZCAssetTypeLivePhoto;
                } else {
                    self.assetType = ZCAssetTypeImage;
                }
                break;
            case PHAssetMediaTypeVideo:
                self.assetType = ZCAssetTypeVideo;
                break;
            case PHAssetMediaTypeAudio:
                self.assetType = ZCAssetTypeAudio;
                break;
            default:
                self.assetType = ZCAssetTypeUnknow;
                break;
        }
    }
    return self;
...
- (UIImage *)previewImage {
    ...
    if (_usePhotoKit) {
        ...
        [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestImageForAsset:_phAsset
                                                                              targetSize:CGSizeMake(kScreenWidth, kScreenHeight)
...
- (NSInteger)requestOriginImageWithCompletion:(void (^)(UIImage *result, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    if (_usePhotoKit) {
        ...
        return [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestImageForAsset:_phAsset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeDefault options:imageRequestOptions resultHandler:^(UIImage *result, NSDictionary *info) {
            if (completion) {
...
- (NSInteger)requestPreviewImageWithCompletion:(void (^)(UIImage *result, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    if (_usePhotoKit) {
        ...
        imageRequestOptions.networkAccessAllowed = YES; // 允许访问网络
        imageRequestOptions.progressHandler = phProgressHandler;
        return [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestImageForAsset:_phAsset targetSize:CGSizeMake(kScreenWidth, kScreenHeight) contentMode:PHImageContentModeAspectFill options:imageRequestOptions resultHandler:^(UIImage *result, NSDictionary *info) {
            if (completion) {
                completion(result, info);
            }
        }];
...
- (NSInteger)requestLivePhotoWithCompletion:(void (^)(PHLivePhoto *livePhoto, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetImageProgressHandler)phProgressHandler {
    if (_usePhotoKit && [[PHCachingImageManager class] instancesRespondToSelector:@selector(requestLivePhotoForAsset:targetSize:contentMode:options:resultHandler:)]) {
        ...
        return [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestLivePhotoForAsset:_phAsset targetSize:CGSizeMake(kScreenWidth, kScreenHeight) contentMode:PHImageContentModeDefault options:livePhotoRequestOptions resultHandler:^(PHLivePhoto * _Nullable livePhoto, NSDictionary * _Nullable info) {
            if (completion) {
...
- (NSInteger)requestPlayerItemWithCompletion:(void (^)(AVPlayerItem *playerItem, NSDictionary<NSString *, id> *info))completion withProgressHandler:(PHAssetVideoProgressHandler)phProgressHandler {
    if (_usePhotoKit && [[PHCachingImageManager class] instancesRespondToSelector:@selector(requestPlayerItemForVideo:options:resultHandler:)]) {
        ...
        return [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestPlayerItemForVideo:_phAsset options:videoRequestOptions resultHandler:^(AVPlayerItem * _Nullable playerItem, NSDictionary * _Nullable info) {
            if (completion) {
...
- (void)requestImageData:(void (^)(NSData *imageData, NSDictionary<NSString *, id> *info, BOOL isGif))completion {
    if (self.assetType != ZCAssetTypeImage && self.assetType != ZCAssetTypeLivePhoto) {
        if (completion) {
...
- (NSString *)assetIdentity {
    ...
    _assetIdentityHash = [identity zc_md5];
    return _assetIdentityHash;
}
...
- (void)requestPhAssetInfo:(void (^)(NSDictionary *))completion {
    ...
    if (self.assetType == ZCAssetTypeVideo) {
        [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestAVAssetForVideo:_phAsset options:NULL resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
...
- (void)requestImagePhAssetInfo:(void (^)(NSDictionary *))completion synchronous:(BOOL)synchronous {
    ...
    [[[ZCAssetsManager sharedInstance] phCachingImageManager] requestImageDataForAsset:_phAsset options:imageRequestOptions resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        if (info) {
            NSMutableDictionary *tempInfo = [[NSMutableDictionary alloc] init];
            if (imageData) {
...
- (void)setDownloadProgress:(double)downloadProgress {
    ...
    _downloadStatus = ZCAssetDownloadStatusDownloading;
}

- (void)updateDownloadStatusWithDownloadResult:(BOOL)succeed {
    _downloadStatus = succeed ? ZCAssetDownloadStatusSucceed : ZCAssetDownloadStatusFailed;
}
