...
void ZCSaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, ZCAssetsGroup *albumAssetsGroup, ZCWriteAssetCompletionBlock completionBlock) {
    [[ZCAssetsManager sharedInstance] saveImageWithImagePathURL:[NSURL fileURLWithPath:imagePath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

void ZCSaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, ZCAssetsGroup *albumAssetsGroup, ZCWriteAssetCompletionBlock completionBlock) {
    [[ZCAssetsManager sharedInstance] saveVideoWithVideoPathURL:[NSURL fileURLWithPath:videoPath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}



@implementation ZCAssetsManager {
    ALAssetsLibrary *_alAssetsLibrary;
    PHCachingImageManager *_phCachingImageManager;
    BOOL _usePhotoKit;
}

+ (ZCAssetsManager *)sharedInstance {
    static dispatch_once_t onceToken;
    static ZCAssetsManager *instance = nil;
...
+ (ZCAssetAuthorizationStatus)authorizationStatus {
    __block ZCAssetAuthorizationStatus status;
    if ([[ZCAssetsManager sharedInstance] usePhotoKit]) {
        // 获取当前应用对照片的访问授权状态
        PHAuthorizationStatus authorizationStatus = [PHPhotoLibrary authorizationStatus];
        if (authorizationStatus == PHAuthorizationStatusRestricted || authorizationStatus == PHAuthorizationStatusDenied) {
            status = ZCAssetAuthorizationStatusNotAuthorized;
        } else if (authorizationStatus == PHAuthorizationStatusNotDetermined) {
            status = ZCAssetAuthorizationStatusNotDetermined;
        } else {
            status = ZCAssetAuthorizationStatusAuthorized;
        }
    } else {
        // 获取当前应用对照片的访问授权状态
...
+ (void)requestAuthorization:(void(^)(ZCAssetAuthorizationStatus status))handler {
    if ([[ZCAssetsManager sharedInstance] usePhotoKit]) {
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus phStatus) {
            ZCAssetAuthorizationStatus status;
        ...
        [[ZCAssetsManager sharedInstance].alAssetsLibrary enumerateGroupsWithTypes:ALAssetsGroupAll usingBlock:nil failureBlock:nil];
        if (handler) {
            handler(ZCAssetAuthorizationStatusNotUsingPhotoKit);
        }
    }
}
...
- (void)enumerateAllAlbumsWithAlbumContentType:(ZCAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbumIfSupported:(BOOL)showSmartAlbumIfSupported usingBlock:(void (^)(ZCAssetsGroup *resultAssetsGroup))enumerationBlock {
    if (_usePhotoKit) {
        ...
        // 创建一个 PHFetchOptions，用于 ZCAssetsGroup 对资源的排序以及对内容类型进行控制
        PHFetchOptions *phFetchOptions = [PHPhotoLibrary createFetchOptionsWithAlbumContentType:contentType];
        
        // 遍历结果，生成对应的 ZCAssetsGroup，并调用 enumerationBlock
        for (NSUInteger i = 0; i < [tempAlbumsArray count]; i++) {
            PHAssetCollection *phAssetCollection = [tempAlbumsArray objectAtIndex:i];
            ZCAssetsGroup *assetsGroup = [[ZCAssetsGroup alloc] initWithPHCollection:phAssetCollection fetchAssetsOptions:phFetchOptions];
            if (enumerationBlock) {
        ...
        [_alAssetsLibrary enumerateAllAlbumsWithAlbumContentType:contentType usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
            if (group) {
                ZCAssetsGroup *assetsGroup = [[ZCAssetsGroup alloc] initWithALAssetsGroup:group];
                if (enumerationBlock) {
                    enumerationBlock(assetsGroup);
                }
            } else {
                                       ...
                                       completionBlock:^ (NSURL *assetURL, NSError *error) {
                                           [_alAssetsLibrary assetForURL:assetURL
                                                             resultBlock:^(ALAsset *asset) {
                                                                 ZCAsset *resultAsset = [[ZCAsset alloc] initWithALAsset:asset];
                                                                 completionBlock(resultAsset, error);
                                                             } failureBlock:^(NSError *error) {
                                                                 NSLog(@"Get ALAsset of image error : %@", error);
                                                                 completionBlock(nil, error);
                                                             }];
                                       }];
                                        ...
                                        albumAssetCollection:albumPhAssetCollection
                                           completionHandler:^(BOOL success, NSDate *creationDate, NSError *error) {
                                               if (success) {
                                                   ...
                                                   NSLog(@"Get PHAsset of video Error: %@", error);
                                                   completionBlock(nil, error);
                                               }
                                           }];
    } else {
                                             ...
                                             completionBlock:^ (NSURL *assetURL, NSError *error) {
                                                 [_alAssetsLibrary assetForURL:assetURL
                                                                   resultBlock:^(ALAsset *asset) {
                                                                       ...
                                                                       NSLog(@"Get ALAsset of video error: %@", error);
                                                                       completionBlock(nil, error);
                                                                   }];
                                             }];
...
@implementation PHPhotoLibrary (ZC)
...
+ (PHFetchOptions *)createFetchOptionsWithAlbumContentType:(ZCAlbumContentType)contentType {
    ...
    switch (contentType) {
        ...
        case ZCAlbumContentTypeOnlyAudio:
...
+ (NSArray *)fetchAllAlbumsWithAlbumContentType:(ZCAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbum:(BOOL)showSmartAlbum {
    ...
    // 创建一个 PHFetchOptions，用于创建 ZCAssetsGroup 对资源的排序和类型进行控制
...
- (void)addImageToAlbum:(UIImage *)image imagePathURL:(NSURL *)imagePathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (image) {
            ...
            NSLog(@"Creating asset with empty data");
            return;
        }
...
- (void)addVideoToAlbum:(NSURL *)videoPathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (!success) {
            NSLog(@"Creating asset of video error: %@", error);
        }
...
@implementation ALAssetsLibrary (ZC)
