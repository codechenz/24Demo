...
@class ZCZoomImageView;
@class ZCZoomImageViewVideoToolbar;

@protocol ZCZoomImageViewDelegate <NSObject>
@optional
- (void)singleTouchInZoomingImageView:(ZCZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)doubleTouchInZoomingImageView:(ZCZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)longPressInZoomingImageView:(ZCZoomImageView *)zoomImageView;
/**
...
/// 是否支持缩放，默认为 YES
- (BOOL)enabledZoomViewInZoomImageView:(ZCZoomImageView *)zoomImageView;

// 可通过此方法调整视频播放时底部 toolbar 的视觉位置，默认为 {25, 25, 25, 18}
// 如果同时设置了 ZCZoomImageViewVideoToolbar 实例的 contentInsets 属性，则这里设置的值将不再生效
 ...
 *  @see viewportRect
 *
 *  ZCZoomImageView 提供最基础的图片预览和缩放功能以及 loading、错误等状态的展示支持，其他功能请通过继承来实现。
 */
@interface ZCZoomImageView : UIView <UIScrollViewDelegate>

@property(nonatomic, weak) id<ZCZoomImageViewDelegate> delegate;
...
// @see ZCZoomImageViewVideoToolbar
@property(nonatomic, strong, readonly) ZCZoomImageViewVideoToolbar *videoToolbar;
...
/**
 *  获取当前正在显示的图片/视频在整个 ZCZoomImageView 坐标系里的 rect（会按照当前的缩放状态来计算）
 */
...
//@property(nonatomic, strong, readonly) ZCEmptyView *emptyView;
...
@interface ZCZoomImageViewVideoToolbar : UIView
...
// 如果同时实现了 ZCZoomImageViewDelegate 的 contentInsetsForVideoToolbar:inZoomingImageView: 方法，则此处设置的值会覆盖掉 delegate 中返回的值
