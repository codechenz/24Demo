...
#define kIconsColor [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:.75/1.0]
...
// generate icon images needed by ZCZoomImageView
// 用于生成 ZCZoomImageView 所需的一些简单的图标图片
@interface ZCZoomImageViewImageGenerator : NSObject
...
@interface ZCZoomImageVideoPlayerView : UIView

@end

static NSUInteger const kTagForCenteredPlayButton = 1;

@interface ZCZoomImageView () <UIGestureRecognizerDelegate>

@property(nonatomic, strong) UIScrollView *scrollView;

// video play
@property(nonatomic, strong) ZCZoomImageVideoPlayerView *videoPlayerView;
...
@implementation ZCZoomImageView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ZCZoomImageView *appearance = [ZCZoomImageView appearance];
...
- (void)setZoomScale:(CGFloat)zoomScale animated:(BOOL)animated {
    if (animated) {
        [UIView animateWithDuration:.25 delay:0.0 options:ZCViewAnimationOptionsCurveOut animations:^{
...
- (void)addPlayerTimeObserver {
    ...
    double interval = .1f;
    __weak ZCZoomImageView *weakSelf = self;
...
- (ZCZoomImageViewVideoToolbar *)videoToolbar {
...
- (void)initVideoPlayerLayerIfNeeded {
    ...
    self.videoPlayerView = [[ZCZoomImageVideoPlayerView alloc] init];
...
- (void)initVideoToolbarIfNeeded {
    ...
    _videoToolbar = ({
        ZCZoomImageViewVideoToolbar * b = [[ZCZoomImageViewVideoToolbar alloc] init];
...
- (void)initVideoCenteredPlayButtonIfNeeded {
    ...
    _videoCenteredPlayButton = ({
        ...
        b.zc_outsideEdge = UIEdgeInsetsMake(-60, -60, -60, -60);
...
@interface ZCZoomImageView (UIAppearance)

@end

@implementation ZCZoomImageView (UIAppearance)
...
+ (void)setDefaultAppearance {
    ZCZoomImageView *appearance = [ZCZoomImageView appearance];
    appearance.videoCenteredPlayButtonImage = [ZCZoomImageViewImageGenerator largePlayImage];
}

@end

@implementation ZCZoomImageVideoPlayerView
...
@implementation ZCZoomImageViewImageGenerator
...
+ (UIImage *)largePlayImage {
    ...
    // circle outside
    CGContextSetFillColorWithColor(context, [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:.25/1.0].CGColor);
...
@implementation ZCZoomImageViewVideoToolbar

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ZCZoomImageViewVideoToolbar *appearance = [ZCZoomImageViewVideoToolbar appearance];
        ...
        self.backgroundColor = [UIColor colorWithRed:.5/255.0 green:255/255.0 blue:0/255.0 alpha:0/1.0]
;
                _playButton = [[UIButton alloc] init];
        self.playButton.zc_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
        [self.playButton setImage:self.playButtonImage forState:UIControlStateNormal];
        [self addSubview:self.playButton];
        
        _pauseButton = [[UIButton alloc] init];
        self.pauseButton.zc_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
...
//        self.slider.thumbColor = UIColorWhite;
        ...
        [self.sliderRightLabel zc_setTheSameAppearanceAsLabel:self.sliderLeftLabel];
...
@interface ZCZoomImageViewVideoToolbar (UIAppearance)

@end

@implementation ZCZoomImageViewVideoToolbar (UIAppearance)
...
+ (void)setDefaultAppearance {
    ...
    appearance.playButtonImage = [ZCZoomImageViewImageGenerator smallPlayImage];
    appearance.pauseButtonImage = [ZCZoomImageViewImageGenerator pauseImage];
}
