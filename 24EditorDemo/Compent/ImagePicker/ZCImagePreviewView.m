...
#import "ZCImagePreviewView.h"
#import "ZCCollectionViewPagingLayout.h"
#import "NSObject+ZCCate.h"

@interface ZCImagePreviewCell : UICollectionViewCell

@property(nonatomic, strong) ZCZoomImageView *zoomImageView;
@end

@implementation ZCImagePreviewCell
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.zoomImageView = [[ZCZoomImageView alloc] init];
...
@implementation ZCImagePreviewView
...
- (void)didInitializedWithFrame:(CGRect)frame {
    _collectionViewLayout = [[ZCCollectionViewPagingLayout alloc] initWithStyle:ZCCollectionViewPagingLayoutStyleDefault];
    ...
    [self.collectionView registerClass:[ZCImagePreviewCell class] forCellWithReuseIdentifier:kImageOrUnknownCellIdentifier];
    [self.collectionView registerClass:[ZCImagePreviewCell class] forCellWithReuseIdentifier:kVideoCellIdentifier];
    [self.collectionView registerClass:[ZCImagePreviewCell class] forCellWithReuseIdentifier:kLivePhotoCellIdentifier];
...
- (void)setCurrentImageIndex:(NSUInteger)currentImageIndex animated:(BOOL)animated {
    ...
    if (currentImageIndex < [self.collectionView numberOfItemsInSection:0]) {
        ...
        NSLog(@"%@ %s，collectionView.numberOfItems = %@, collectionViewDataSource.numberOfItems = %@, currentImageIndex = %@", NSStringFromClass([self class]), __func__, @([self.collectionView numberOfItemsInSection:0]), @([self.collectionView.dataSource numberOfSectionsInCollectionView:self.collectionView]), @(_currentImageIndex));
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(imagePreviewView:assetTypeAtIndex:)]) {
        ZCImagePreviewMediaType type = [self.delegate imagePreviewView:self assetTypeAtIndex:indexPath.item];
        if (type == ZCImagePreviewMediaTypeLivePhoto) {
            identifier = kLivePhotoCellIdentifier;
        } else if (type == ZCImagePreviewMediaTypeVideo) {
            identifier = kVideoCellIdentifier;
        }
    }
    ZCImagePreviewCell *cell = (ZCImagePreviewCell *)[collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    ZCZoomImageView *zoomView = cell.zoomImageView;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ZCImagePreviewCell *previewCell = (ZCImagePreviewCell *)cell;
...
@implementation ZCImagePreviewView (ZCZoomImageView)

- (NSUInteger)indexForZoomImageView:(ZCZoomImageView *)zoomImageView {
    if ([zoomImageView.superview.superview isKindOfClass:[ZCImagePreviewCell class]]) {
        return [self.collectionView indexPathForCell:(ZCImagePreviewCell *)zoomImageView.superview.superview].item;
    } else {
        NSAssert(NO, @"尝试通过 %s 获取 ZCZoomImageView 所在的 index，但找不到 ZCZoomImageView 所在的 cell，index 获取失败。%@", __func__, zoomImageView);
    }
    return NSUIntegerMax;
}

- (ZCZoomImageView *)zoomImageViewAtIndex:(NSUInteger)index {
    ZCImagePreviewCell *cell = (ZCImagePreviewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    return cell.zoomImageView;
}

- (void)checkIfDelegateMissing {
#ifdef DEBUG
    [NSObject zc_enumerateProtocolMethods:@protocol(ZCZoomImageViewDelegate) usingBlock:^(SEL selector) {
        if (![self respondsToSelector:selector]) {
            NSAssert(NO, @"%@ 需要响应 %@ 的方法 -%@", NSStringFromClass([self class]), NSStringFromProtocol(@protocol(ZCZoomImageViewDelegate)), NSStringFromSelector(selector));
        }
    }];
#endif
}
...
- (void)singleTouchInZoomingImageView:(ZCZoomImageView *)imageView location:(CGPoint)location {
...
- (void)doubleTouchInZoomingImageView:(ZCZoomImageView *)imageView location:(CGPoint)location {
    [self checkIfDelegateMissing];
    if ([self.delegate respondsToSelector:_cmd]) {
...
- (void)longPressInZoomingImageView:(ZCZoomImageView *)imageView {
    [self checkIfDelegateMissing];
    if ([self.delegate respondsToSelector:_cmd]) {
...
- (BOOL)enabledZoomViewInZoomImageView:(ZCZoomImageView *)imageView {
    [self checkIfDelegateMissing];
    if ([self.delegate respondsToSelector:_cmd]) {
...
- (UIEdgeInsets)contentInsetsForVideoToolbar:(ZCZoomImageViewVideoToolbar *)toolbar inZoomingImageView:(ZCZoomImageView *)zoomImageView {
    [self checkIfDelegateMissing];
    if ([self.delegate respondsToSelector:_cmd]) {
